...
* @since   2019-06-27 
*/
public class WorkerThreadForRocksDB implements Runnable {
	 ...
	 String logPath ;
	 String cbpid;
	 private static final Logger LOG = LoggerFactory.getLogger(WorkerThreadForRocksDB.class);
	 
	WorkerThreadForRocksDB(List<File> listOfFiles, List<String> listOfFilesToBeInserted,
		...
		for (File file : listOfFiles) {
			...
			if (file.isFile() && file.length() > 0 && checkFilePresent(file)) {	
					try {			
						...
						InputFile2RockDB obj = new InputFile2RockDB(dumpFilePath, schemaFilePath, filesToBeInserted, dbPath);	
						obj.loadIntoRocksDB(entityName, file, dbPath);
